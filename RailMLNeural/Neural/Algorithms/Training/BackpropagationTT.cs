...
{
    class BackpropagationTT : PropagationThroughTime, ILearningRate
    {
        /// <summary>
        ...
        public BackpropagationTT(BasicNetwork network, IList<IMLDataSet> training) : this(network, training, 0, 0)
        {
            AddStrategy(new SmartLearningRate());
            AddStrategy(new SmartMomentum());
        }

        public BackpropagationTT(BasicNetwork network,
                               IList<IMLDataSet> training, double learnRate,
                               double momentum)
            : this(network, training, learnRate, momentum, 0)
        {
        ...
        public BackpropagationTT(BasicNetwork network,
                               IList<IMLDataSet> training, double learnRate,
                               double momentum, double PerformanceRatio) : base(network, training)
        {
            _performanceRatio = PerformanceRatio;
            _momentum = momentum;
        ...
        {
            double delta = (((1-_performanceRatio)*gradients[index] - _performanceRatio * Network.Flat.Weights[index]) * _learningRate)
